...
// Dot product between `lhs` (n_bits lanes) and a bit-packed int8 vector of
// ceil(n_bits/8) bytes, without unpacking the bits into a separate array
// first. See `BitDotProductF32Fn`/`BitDotProductF64Fn` in fn_table.h.
[[nodiscard]] inline double bit_dot_product(const float* lhs, const int8_t* packed_bits, size_t n_bits,
                                            bool big_bitorder) noexcept {
    return dispatch::VESPA_HWACCEL_DISPATCH_FN_PTR_NAME(bit_dot_product_f32)(lhs, packed_bits, n_bits, big_bitorder);
}

[[nodiscard]] inline double bit_dot_product(const double* lhs, const int8_t* packed_bits, size_t n_bits,
                                            bool big_bitorder) noexcept {
    return dispatch::VESPA_HWACCEL_DISPATCH_FN_PTR_NAME(bit_dot_product_f64)(lhs, packed_bits, n_bits, big_bitorder);
}
